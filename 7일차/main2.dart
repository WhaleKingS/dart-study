...
Future main() async {
  ...
  await for (HttpRequest request in server) {
    ...
    //try 뒤에 있는 {}내용을 실행하되, 해당 내용 실행 중 에러가 발생하면
    //catch 이후에 있는  {} 의 내용으로 바꿔서 실행해라.

    try {
      //http://127.0.0.1:4040/ 에서 ip와 port 를 제외한 이후 문자열이 / 만 있는지 확인하는 조건문
      // request.uri.path -> 위의 url 에서 http://127.0.0.1:4040 을 제외한 그 다음 문자열 즉/
      if (request.uri.path == '/') {
        ...
        // .substring(1) -> 맨 앞에 잇는 첫번째 문자열을 제거하겠다. 즉 sample.txt
